...
A hybrid Formula 1 assistant that routes questions to the right data source: live telemetry (OpenF1), historical race results (Kaggle CSVs), or FIA regulation documents (RAG over PDFs).
...
- **Historical stats** — driver teams, race results, gaps, and podium finishes from 1950–2020 CSV data
...
# (Optional) Download historical CSVs if not already present
...
- `Which team did Lewis drive for in the 2008 British GP?` → historical CSV database
...
utils/
  ...
  historical_db.py      # Historical CSV query layer
data/
